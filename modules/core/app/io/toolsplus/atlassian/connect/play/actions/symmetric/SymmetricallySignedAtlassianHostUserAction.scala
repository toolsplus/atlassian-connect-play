...
case class ConnectAtlassianHostUserRequest[A](
...
class SymmetricallySignedAtlassianHostUserAction @Inject() (
  ...
  /** Creates an action builder that validates symmetrically signed JWT
    * requests. Callers must specify how the query string hash claim should be
    * verified.
    *
    * @param qshProvider
    *   Query string hash provider that specifies what kind of QSH the qsh claim
    *   contains
    * @return
    *   Play action for symmetrically signed JWT requests
    */
  def authenticateWith(
      qshProvider: QshProvider
  ...
  object Implicits {
    ...
    /** Implicitly convert an instance of [[ConnectAtlassianHostUserRequest]] to
      * an instance of AtlassianHostUser.
      *
      * @param request
      *   Atlassian host user request instance.
      * @return
      *   Atlassian host user instance extracted from request.
      */
    implicit def hostUserRequestToHostUser(implicit
        request: ConnectAtlassianHostUserRequest[_]
    ): AtlassianHostUser =
      request.hostUser
