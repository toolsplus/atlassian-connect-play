...
class LifecycleService @Inject() (
  ...
  /** Installs the given Atlassian host by saving the security context to the
    * database.
    *
    * @param installedEvent
    *   Lifecycle event holding the security context provided by Atlassian
    *   product.
    * @return
    *   Installed AtlassianHost instance.
    */
  def installed(
      installedEvent: InstalledEvent
  ...
  /** Extracts the Atlassian host from the installed event and saves it to the
    * database.
    *
    * @param installedEvent
    *   Lifecycle event holding the security context provided by Atlassian
    *   product.
    * @return
    *   Either the newly installed [[AtlassianHost]] or a [[HostForbiddenError]]
    */
  ...
  /** Uninstalls the given Atlassian host by setting the installed flag in the
    * host store to false. If the configuration property
    * `atlassian-connect.hostTTLDays` is set, it will also mark the host with a
    * TTL value.
    *
    ...
    * treated as successful uninstallation as the host is not there anyway.
    *
    * @param uninstalledEvent
    *   Lifecycle event payload.
    * @param maybeHostUser
    *   Installed Atlassian host user if one has been found.
    * @return
    *   Either [[LifecycleError]] or AtlassianHost.
    */
  def uninstalled(
      uninstalledEvent: GenericEvent,
  ...
  /** Uninstalls the Atlassian host indicated by the uninstall event payload.
    *
    * Note that if there is an installed host record found, the host client key
    * in the given payload will be verified against the installed host client
    * key. Uninstallation will only succeed if the two client key values match.
    *
    * @param uninstalledEvent
    *   Uninstall event payload
    * @param maybeExistingHost
    *   Atlassian host associated with the JWT request if one has been found
    * @return
    */
  private def uninstall(
      uninstalledEvent: GenericEvent,
      maybeExistingHost: Option[AtlassianHost]
  ): EitherT[Future, LifecycleError, AtlassianHost] = {
    maybeExistingHost match {
      case Some(host) =>
        val ttl = connectProperties.hostTTLDays.map(ttlDays =>
          Instant.now().plus(Duration.ofDays(ttlDays))
        )
        logger.info(
          s"Saved uninstallation for host ${host.baseUrl} (${host.clientKey}) (TTL: ${ttl.map(_.toString).getOrElse("none")})"
        )
        assertHostAuthorized(uninstalledEvent, host)
          .toEitherT[Future]
          .flatMap(host =>
            EitherT.right(hostRepository.save(host.uninstalled(ttl)))
          )
      case None =>
        logger.error(
  ...
  ): Either[LifecycleError, LifecycleEvent] = {
  ...
  private def assertHostAuthorized(
      lifecycleEvent: LifecycleEvent,
      host: AtlassianHost
  ): Either[LifecycleError, AtlassianHost] = {
