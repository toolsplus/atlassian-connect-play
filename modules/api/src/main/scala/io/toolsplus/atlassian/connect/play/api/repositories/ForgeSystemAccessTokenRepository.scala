...
trait ForgeSystemAccessTokenRepository {
  ...
  /** Saves the given Forge system access token by inserting it if it does not exist or
    ...
    *
    * @param token Forge system access token to store.
    * @return Saved Forge system access token.
    */
  def save(token: ForgeSystemAccessToken): Future[ForgeSystemAccessToken]

  /**
    * Finds the access token for the given installation ID. If an access token is returned,
    * it may be expired.
    *
    * @param installationId ID of the installation associated with the access token
    * @return Access token and with metadata, if one is found.
    */
  ...
  /**
    ...
    *
    * @param installationId ID of the installation associated with the access token
    * @param expirationTime In most cases, specify the timestamp as of the moment this method is called with some leeway
    * @return Valid access token and with metadata, if one is found.
    */
